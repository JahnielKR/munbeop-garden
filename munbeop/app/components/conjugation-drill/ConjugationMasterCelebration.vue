...
/** One-shot reward card when 활용 마스터 is earned. No pixel-art asset. */
interface Props {
  total: number
}
defineProps<Props>()
...
<template>
  <div
    class="cel"
    data-testid="conj-master-celebration"
  ...
  >
    <section
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="conj-cel-title"
      aria-describedby="conj-cel-body"
    >
      <span class="cel__badge" aria-hidden="true">🏅</span>
      <h2 id="conj-cel-title" class="cel__title" lang="ko">활용 마스터!</h2>
      <p class="cel__label">{{ t('conjugation.master.label') }}</p>
      <p id="conj-cel-body" class="cel__body">
        {{ t('conjugation.master.celebrate_body', { total }) }}
      </p>
      <button
        ref="dismissBtn"
        type="button"
