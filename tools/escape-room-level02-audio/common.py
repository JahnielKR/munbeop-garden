#!/usr/bin/env python3
"""Shared DSP toolkit for the Level 2 escape-room AUDIO pipeline.

Every gen_*.py audio script imports from here so the whole soundscape of
"El templo de la lluvia (청우사)" shares ONE format standard (44100 Hz, MONO,
float32 in (-1, 1), written as OGG/Vorbis) and ONE house DSP vocabulary.

Tone (per docs/escape-room-level-02.md and tools/escape-room-level02/STYLE.md):
melancolía luminosa, duelo sereno bajo lluvia — "El jardín de las palabras" ×
"Mushishi". NOT horror, NOT retro-arcade, NOT melodrama. Sounds are soft,
low-passed, patient. The one bright sound in the level is THE bell (범종): a
bronze strike with a long beating 여음 (afterglow) that the whole story leans on.

Final OGGs land under munbeop/public/escape-room/level-02/audio/ (ambient beds +
SFX at the top level, Korean TTS voice lines under audio/voice/). Previews,
debug renders and analysis dumps go under tools/escape-room-level02-audio/out/
...
REPO = HERE.parents[1]
AUDIO_DIR = REPO / "munbeop" / "public" / "escape-room" / "level-02" / "audio"
